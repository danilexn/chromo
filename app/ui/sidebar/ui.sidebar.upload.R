chromo.ui.sidebar.upload <- function(request) {
  conditionalPanel(
    ...
    conditionalPanel(
      ...
      # From https://github.com/JoachimGoedhart/PlotsOfData/blob/master/app.R
      conditionalPanel(
        condition = "input.adjustcolors == '2'",
        textInput(
          "user_color_list",
          "Comma separated names or hexadecimal codes. These are applied to factors, in alphabetical order",
          value = "salmon,#00BFC4,chartreuse3"
        ),
        h5(
          "",
          a("More info on color names",
            href = "https://r-charts.com/colors/", target =
              "_blank")
        )
